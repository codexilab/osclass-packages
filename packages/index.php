...
osc_add_hook('admin_menu_init', function() {
	...
	osc_add_admin_submenu_page(
		'plugins', __('Settings'), osc_route_admin_url('packages-admin-settings'), 'packages-admin-settings', 'administrator'
	);
...
function custom_user_bulk_options($bulk_options) {
	//$bulk_options[] = array('value' => 'assign_package', 'data-dialog-content' => sprintf(__('Are you sure you want to %s(s)?'), strtolower(__('Assign package', 'packages'))), 'label' => __('Assign package', 'packages'));
...
function packages_custom_css_admin() {
	...
	if (Params::getParam('route') == 'packages-admin') {
		osc_enqueue_style('packages-css', osc_base_url() . 'oc-content/plugins/'. PACKAGES_FOLDER. 'assets/css/admin/main.css');
	}
}
