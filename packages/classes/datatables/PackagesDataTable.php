 ...
 */
	...
	class PackagesDataTable extends DataTable
	{
		...
		private function addTableHeader()
		{
			...
			$this->addColumn('user-type', __('User type'));
			...
			$this->addColumn('update-date', __('Update date'));
		...
		private function processData($packages)
		{
		    if(!empty($packages)) {
				...
				foreach($packages as $aRow) {
				    ...
				    $options[]  = '<a href="'.osc_route_admin_url('packages-admin').'&package='.$aRow['pk_i_id'].'">' . __('Edit') . '</a>';
				    $options[]  = '<a href="#" onclick="action_dialog(\'package-delete\', '.$aRow['pk_i_id'].');return false;">' . __('Delete') . '</a>';

				    if( $aRow['b_active'] == 1 ) {
						$options[]  = '<a href="#" onclick="action_dialog(\'package-deactivate\', '.$aRow['pk_i_id'].');return false;">' . __('Deactivate') . '</a>';
				    } else {
						$options[]  = '<a href="#" onclick="action_dialog(\'package-activate\', '.$aRow['pk_i_id'].');return false;">' . __('Activate') . '</a>';
				    }
				    ...
				    $row['user-type']       = ($aRow['b_company'] == 0) ? __('User') : __('Company');
